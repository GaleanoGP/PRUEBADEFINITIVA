...
<body class="js">
          ...
          <div class="col-12">
            <div class="bread-inner">
              <ul class="bread-list">
                ...
                <li class="active"><a href="cart.php">Pedido Realizado</a></li>
              </ul>
            </div>
          </div>
        </div>
    ...
    // compruebo si el valor de la superglobal SESSION está vacío
    if (empty($_SESSION['correo'])) {
      // si no existe ningun valor asignado a la variable correo, indicamos al usuario que debe iniciar sesión para hacer pedidos
      echo "Debes iniciar sesión para hacer pedidos";
    }
    else {
        if(isset($_POST['name']) AND count($_POST['name']) > 0) {
          //establecemos conexión con la base de datos.
            $conexion = new mysqli($host_db, $user_db, $pass_db, $db_name);
            //guardamos en la tabla pedidos el correo y el total del pedido.
            $query = "INSERT INTO pedidos (correo,total) VALUES ('$correo','0')";
            $res = $conexion->query($query);
            $order_id = mysqli_insert_id($conexion);
            $total = 0;
            //recorremos con un bucle for todos los productos.
            for ($i = 0; $i < count($_POST['name']); $i++) {
                ...
                $query = "INSERT INTO order_details (order_id,nombre, precio, cantidad) VALUES ('$order_id','$nombre','$precio','$cantidad')";
                $res = $conexion->query($query);
            }
            //hacemos la actualización de la tabla pedidos.
            $query = "UPDATE pedidos SET total='$total' WHERE id='$order_id'";
            $res = $conexion->query($query);
            //le damos las gracias al usuario por haber realizado el pedido.
            if ($res === TRUE) {
                echo "<a href='mostrarproductos.php'>Gracias por tu pedido !. </a>";
            } else {
                echo "Inicia Sesion para realizar pedidos.<br>";
                echo "<br><a href='login.php'>Login</a>";
                exit;
            }
        }else{
            echo "No se encontró ningún producto.<br>";
            echo "<br><a href='login.php'>Login</a>";
            exit;
        }
      $now = time();
